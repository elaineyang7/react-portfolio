...
const ProjectCard = ({ classname, project }) => {
  ...
  if (classname === 'project') {
    ...
    projectAnimation = ""
    partAnimation = "fade-up"
  } else {
    span = ""
    heading = <h4>{project.title}</h4>
    projectAnimation = "fade-up"
    partAnimation = ""
  }
  ...
  return(
    <div 
      className={classname} 
      data-aos={projectAnimation} 
      data-aos-duration="1500" 
      data-aos-once="true"
    >
      <div 
        className={classname + `__imgBox`} 
        data-aos={partAnimation} 
        data-aos-duration="1000" 
        data-aos-once="true"
      >
        <img src={project.image_link} alt={project.title} loading="lazy" />
      </div>
      <div 
      ...
      >
        ...
        <div className="skills">
          {project.skills.map( (skill, index) => (
            <SkillButton 
              key={index} 
              classname={classname + `__skillBtn`} 
              skill={skill} />
          ) )}
        </div>
        <div class="view">
          <a 
            className={classname + `__viewMore`} 
            href={project.view}
            target="_blank"
          >
            View More
